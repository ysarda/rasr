...
def runFunction(sites, dateList, timerange, static_dir):
    ...
    try:
        ...
        for single_date in dateRange(
            start_date, end_date
        ):  # --OPTION 2 DOWNLOAD WEATHER DATA FROM THE ORIGIN DATE TILL TODAY (usually run for the first time to download all the files, and then go to OPTION 2)
        ...
        date_info = "The last attempted download date was in the following format:" "  MONTH / DAY / YEAR:    " + str(
            month
        ) + "/" + str(
            day
        ) + "/" + str(
